...
async function updateProjectCount() {
  try {
    const projects = await currentProjects.length;
    countVal.innerText = projects;
  } catch (err) {
    countVal.innerText = 0;
    console.error("count display error:", err);
  }
}
updateProjectCount();
...
function openNewEntryDialog() {
  ...
  if (getComputedStyle(createEntryDialog).display == "none" || getComputedStyle(createEntryDialog) != "flex") {
